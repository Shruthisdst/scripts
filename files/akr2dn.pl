...
$GrB = "46|54|53|2A|ED|EC|22|5B|7B|6F|6A|6E|55|7D|BC|F9|E4|FA|B9|B2|D3|B1|DC|B6|DA|DB|E0|D7|B7|C6|C2|EA|CE|C7|C3|FF|F7|E4|B2|B1|AB|FE";
%GrB_a = (
'46'=>'i',
'54'=>'U',
'53'=>'e',
...
for($i1=0;$i1<@list1;$i1++)
{
	chop($list1[$i1]);
	$outfile = $list1[$i1];
	$outfile =~ s/_out\.txt/_uni.txt/;

	print $list1[$i1] . "-->" . $outfile . "\n";
	
	open(IN,"<:encoding(UTF-8)","$list1[$i1]") or die "can't open $list1[$i1]";
	...
	while($line)
	{
		...
		elsif($line =~ /"(.*)"/)
		{
			...
			open(TMP,">uni.txt") or die "can't open uni.txt";
			print TMP $final_str;
			close(TMP);
			system("txtconv -t velthuis_1.tec -i uni.txt -o uni1.txt");
			open(TMP,"uni1.txt") or die "can't open uni1.txt";
			$final_str = <TMP>;	
			close(TMP);				
			$line =~ s/"(.*)"/"$final_str"/;			
			print OUT $line . "\n";			
